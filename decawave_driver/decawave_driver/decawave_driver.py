...
class DecawaveDriver(Node):
    ...
    def __init__(self):
        ...
        self.serial_timeout_param = self.get_parameter('serial_timeout').get_parameter_value().double_value
    ...
    def get_uart_mode(self):
        ...
        if cc == '\r\n' and self.ser.readline() == 'dwm> ': # SHELL MODE
            self.get_logger().info("\33[96mDevice is on SHELL MODE! It must to be changed to GENERIC MODE!\33[0m")
            self.mode_ = "SHELL"
    ...
    def switch_uart_mode(self):
        if self.mode_ == "SHELL":
            ...
            self.get_logger().warn("\33[96mUnknown Mode Detected! Please reset the device and try again!\33[0m")
    ...
    def get_tag_version(self):
        ...
        self.get_logger().info("\33[96mConfiguration Version:0x"+format(data_[8], '04X')+"\33[0m")
    ...
    def get_tag_status(self):
        ...
        if data_[0] != 64 and data_[2] != 0:
            ...
            print("%s", data_)
        if data_[5] == 3:
            self.get_logger().warn("\33[96mTag is CONNECTED to a UWB network and LOCATION data are READY!\33[0m")
        elif data_[5] == 2:
            self.get_logger().warn("Tag is CONNECTED to a UWB network but LOCATION data are NOT READY!")
        elif data_[5] == 1:
            self.get_logger().warn("Tag is NOT CONNECTED to a UWB network but LOCATION data are READY!")
        elif data_[5] == 0:
            self.get_logger().warn("Tag is NOT CONNECTED to a UWB network and LOCATION data are NOT READY!")
    ...
    def get_tag_location(self):
        ...
        for i in range(self.tag.n_anchors):
            ...
            a.header.stamp = self.get_clock().now()
